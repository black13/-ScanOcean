...
protected:
   ...
   // Image size; width = dim[0] ; height = dim[1]
   int m_dim[2];

   // Pointer to original image buffer
   mutable char* m_buffer;



   uint32 bitsPerPixel;  // Bits per pixel
   uint32 fill ;          // Fill order
   uint32 imageWidth ;    // Image width in pixels
   uint32 imageHeight ;   // Image height in rows

   uint32 numStrips ;     // Number of strips

   uint32 scanlineSize ;  // Scan line size in bytes
   uint32 stripSize ;     // Strip size

   mutable boost::shared_array<char> buffer;
