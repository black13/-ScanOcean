...
void ROITableWidget::addROI()
{
   ...
   QString text;
   text = QString("ROI %1").arg(row);

   // Add data
   m_model -> addROI(text, m_colors.at(m_colorIndex), -1.0, -1.0, -1.0, -1.0);
...
void ROITableWidget::copyROI()
{
   ...
   if (row >= 0 && row < m_model -> rowCount(QModelIndex())) {
      ...
      // Get index of new ROI
      int i = m_model -> rowCount(QModelIndex()) + 1;

      // Create text description
      QString text;
      text = QString("ROI %1").arg(i);
      ...
      // Add data
      m_model -> addROI(text, m_colors.at(m_colorIndex), x, y, w, h);
