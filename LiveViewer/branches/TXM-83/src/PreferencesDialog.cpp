   ...
   : QDialog(parent, f)
{
   ...
   QListWidgetItem* histButton = new QListWidgetItem(m_contentsWidget);
   histButton->setText(tr("Histogram"));
   histButton->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
   ...
   m_histPage = new PreferencesHist();
   m_pagesWidget->addWidget(m_histPage);
...
void PreferencesDialog::accept()
{
   ...
   if (m_preferences != NULL) {
      ...
      //                              m_roiPage -> getROIPVs());
      m_preferences->saveValueKey(Preferences::HISTBins,
                                  m_histPage->getHistBins());
      m_preferences->saveValueKey(Preferences::HISTType,
                                  m_histPage->getHistType());
   }
...
void PreferencesDialog::setPreferences(Preferences* preferences)
{
   ...
   if (m_preferences != NULL) {
      ...
      m_histPage->setHistBins(preferences->readValueKey(
               Preferences::HISTBins).toDouble());
      m_histPage->setHistType(preferences->readValueKey(
               Preferences::HISTType).toInt());
   }
