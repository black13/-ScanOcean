...
void LiveViewer::showPreferences()
{
   ...
   int histType = m_preferences.readValueKey(Preferences::HISTType).toInt();
   m_colorTransferFunctionEditorWidget->updateHistogramType(histType);
...
void LiveViewer::startThread()
{
   ...
   connect(m_displayStage, SIGNAL(newFrame(const Array*)),
           this, SLOT(updateFrame(const Array*)));
...
void LiveViewer::stopThread()
{
   ...
   disconnect(m_displayStage, SIGNAL(newFrame(const Array*)),
              this, SLOT(updateFrame(const Array*)));
