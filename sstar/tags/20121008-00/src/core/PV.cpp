...
void PV::connectionAttempt(unique_lock<mutex>& localLock,long timeoutSeconds)
{
      ...
      == PVSharedResources::DISCONNECTED)
   {
      ...
      if(timeoutSeconds > 0)
      {
         ...
         m_pSharedResources->getConnectionCV().timed_wait(
            localLock,
...
void PV::asynGet(int n, CAEventHandler* pHandler, bool flush)
{
   ...
   boost::unique_lock<boost::mutex> lock(m_pSharedResources->getLocalMutex());

   IndentedWriter w = Logger::getIndentedWriter("PV: AsynGet");

   int result;

   result = ca_array_get_callback(ca_field_type(m_pSharedResources->getChid()),
                                  n,
                                  m_pSharedResources->getChid(),
         ...
         if(m_pSharedResources->getDataChangeHandlers().empty() == true)
         {
            ...
            while( done == false)
            {
               ...
               catch(ChannelAccessException& e)
               {
                  ...
                  if(!m_pSharedResources->getConnectionCV().timed_wait
                           (localLock,
