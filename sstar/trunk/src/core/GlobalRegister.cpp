...
shared_ptr<PV> GlobalRegister::getPV(const string& name)
{
   ...
   w() << "Returning a Smart Pointer to a newly created PVInterface"
       << std::endl;
   return std::shared_ptr<PV>(new PV(pResources));

}

/*---------------------------------------------------------------------------*/

/*std::list<std::string> GlobalRegister::getPVNames()
{

}*/
std::list<std::shared_ptr<PVSharedResources> >
GlobalRegister::getSharedResourcesList()
   {
      return m_sharedResourcesTable.getValues();
   }
...
void GlobalRegister::disposeResources(const string& name)
{
   ...
   if(m_threadAccessTable.contains(name))
   {
	   std::list<std::thread::id> threads = m_threadAccessTable.get(name);
