...
public:
    ...
    */
   PV(const std::shared_ptr<PVResources>& pResources);
   ...
   void synPut(int ca_type, void* pData, int n);
   ...
   void asynGet(int ca_type, int n, CAEventHandler* pHandler, bool flush = true);
   ...
   /**
    * Same as synGet but you define the type for the PV. Usefull for 
    *  getting the string value of enum.
    */
   void synGet(int type, int n, CAEventHandler* pHandler);
...
private:
    ...
    */
   void connectionAttempt(std::unique_lock<std::mutex>& localLock,
                          long timeoutSeconds = 5);
