...
private:
    ...
    * to it next time. 
    */
   std::shared_ptr<PV> getPV(const std::string& pvName);
    ...
    */
   void* getValue(const Command* cmd, std::shared_ptr<PV> pv);
   ...
   void waitPVCallback(std::shared_ptr<PV> pv, const Command* cmd);

   /**
    * Helper function to wait on a PV value to be set to the one
    * specified in Command.
    */
   void waitPVSet(std::shared_ptr<PV> pv, const Command* cmd);
...
private:
   ...
   std::condition_variable m_pvCallback;

   std::map<std::string, std::shared_ptr<PV> > m_pvList;
