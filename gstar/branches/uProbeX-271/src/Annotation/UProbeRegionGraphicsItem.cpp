...
UProbeRegionGraphicsItem* UProbeRegionGraphicsItem::cloneRegion()
{
   ...
   m_outlineColor = uProbeRegion->m_outlineColor;

   m_rect = uProbeRegion->m_rect;

   setSize(uProbeRegion->m_size);
...
int UProbeRegionGraphicsItem::predictFontPixelSizeByScale(qreal scale) {
   ...
   if (scale < 0.15)
   {
      pixelSizeScalingFactor = 100.0;
   }
   else if (scale < 0.20)
   {
      pixelSizeScalingFactor = 75.0;
   }
   else if (scale < 0.30)
   {
      pixelSizeScalingFactor = 50.0;
   }
   else if (scale < 0.50)
   {
      pixelSizeScalingFactor = 25.0;
   }
   else if (scale < 0.75)
   {
      pixelSizeScalingFactor = 20.0;
   } else
   {
      pixelSizeScalingFactor = 12.0;
   }
