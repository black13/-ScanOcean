...
/*---------------------------------------------------------------------------*/
void ImageViewWidget::setWidgetAbleStatus(bool m_rangeEnable)
{
...
void ImageViewWidget::createRangewidget()
{
   ...
   //range -> setId(0);
   m_range -> setMaximum(13);
   m_range -> setMinimum(0);
   connect(m_range, SIGNAL(valueChanged(int, int)),
   this, SLOT(rangeChanged(int, int)));
   m_toolbar->addWidget(m_range);

   QLabel* m_widlbl = new QLabel(" Width :");
   m_toolbar->addWidget(m_widlbl);
   m_comboWidth = new QComboBox();
   m_comboWidth->setEditable(true);
   connect(m_comboWidth, SIGNAL(valueChangedWidth(int)),
           this, SLOT(comboWidthChanged(int)));
   m_toolbar->addWidget(m_comboWidth);


   QLabel* m_higlbl = new QLabel(" Height :");
   m_toolbar->addWidget(m_higlbl);

   m_comboHeight = new QComboBox();
   m_comboHeight->setEditable(true);
   connect(m_comboHeight, SIGNAL(valueChangedHeight(int)),
           this, SLOT(comboHeightChanged(int)));
   m_toolbar->addWidget(m_comboHeight);
   m_toolbar->addSeparator();
